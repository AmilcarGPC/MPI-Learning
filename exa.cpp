...
int main(int argc, char *argv[]) {
    ...
    for (i = 0; i < NxG; i++)
        for (j = 0; j < NyG; j++){
            UG[i*NxG+j] = 3.0 * sin(pi * xG[i] + pi * yG[j]) * sin(pi * xG[i] + pi * yG[j]);
        }
            
    // Condiciones de frontera
    for (j = 0; j < NyG; j++) {
        UG[j] = 2.0;     // Oeste
        UG[(NxG - 1)*NxG+j] = 1.0; // Este
    }
    for (i = 0; i < NxG; i++) {
        UG[i*NxG] = 1.0;     // Sur
        UG[i*NxG+NyG - 1] = 3.0; // Norte
    }
    ...
    Ut   = (double *)malloc(Nx * Ny * sizeof(double));
    ...
    for (i = 0; i < Nx; i++) {
        ...
        for (j = 0; j < Ny; j++) {
            j_index = index_global_j[j];
            Uold[i*Ny+j] = UG[i_index*NxG+j_index];
        }
    }
    ...
    for (int i = 0; i < Nx; i++) {
        for (int j = 0; j < Ny; j++) {
            Unew[i][j] = Uold[i*Ny+j];
            Ut[i*Ny+j] = Uold[i*Ny+j];
        }
    }
            ...
            if (Nx <= PARAVIEW_MAX_GRID) {
                ...
                if (taskid == numtasks-1) {
                    ...
                    if (coords[0] == 0){
                        for (int j = 0; j < MdomF; j++) {
                            for (int i = 0; i < NdomF; i++) {
                                UG[(iIni+i)*NxG+jIni+j-1] = Unew[i][j];
                            }
                        }
                    }
                    ...
                    if (coords[1] == 0){
                        for (int j = 0; j < MdomF; j++) {
                            for (int i = 0; i < NdomF; i++) {
                                UG[(iIni+i)*NxG+jIni+j] = Unew[i+1][j];
                            }
                        }
                    }
                    ...
                    if ((coords[0] != 0) && (coords[1] != 0)){
                        for (int j = 1; j < MdomF + 1; j++) {
                            for (int i = 1; i < NdomF + 1; i++) {
                                UG[(iIni+i-1)*NxG+jIni+j-2] = Unew[i][j-1];
                            }
                        }
                    }
                    ...
                    for (int i = 0; i < numtasks-1; i++) {
                        ...
                        for (int c = 0; c < lNx; c++){
                            ...
                            if (lcoords[1] == 0){
                                for (int j = 0; j < lNy; j++) {
                                    UG[(iIni + c)*NxG+jIni + j] = received_column[j];
                                }
                            } else {
                                for (int j = 1; j < lNy + 1; j++) { //lNy + 1 causa desbordes
                                    UG[(iIni + c)*NxG+jIni + j - 2] = received_column[j-1];
                                }
                            }
                        }
